...
import {
  ...
  Repeat, Compass, FileText, Receipt, SlidersHorizontal, X,
} from "lucide-react";
...
const MAIS = [
  ...
  { href: "/contracheque", icon: FileText, label: "Contracheque", desc: "Simulação e registro mensal" },
...
export default function NavBar() {
  ...
  return (
    <>
      {/* Menu "Mais" — sheet inferior */}
      {maisAberto && (
        <div className="fixed inset-0 z-40" style={{ background: "rgba(0,0,0,0.5)" }}
          onClick={() => setMaisAberto(false)}>
          <div className="absolute bottom-0 left-0 right-0 rounded-t-2xl p-4 pb-24 flex flex-col gap-1"
              ...
              return (
                <Link key={href} href={href} onClick={() => setMaisAberto(false)}
                  ...
                  <span className="flex flex-col">
                    <span className="text-sm font-medium" style={{ color: ativo ? "var(--primary)" : "var(--text)" }}>{label}</span>
